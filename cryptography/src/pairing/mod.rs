...
/// ```
pub fn contact_message() -> Result<(PairingContactMessageMaterial, PairingSecretKeyMaterial), DerecPairingError> {
...
mod tests {
    ...
    fn test_pairing() {
        // generated by Bob
        let (bob_contact, bob_secrets) = contact_message().unwrap();
        let (alice_request, alice_secrets) = pairing_request_message(&bob_contact).unwrap();
