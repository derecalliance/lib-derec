...
/// Machine-readable schema for consumers that generate their own code from
/// the DeRec protocol definitions.
///
/// Gated behind the `descriptor` feature because the descriptor set is
/// ~122 KB. Unconditional inclusion would add that to every binary built on
/// this crate, including the WebAssembly bundles the Node.js and web SDKs
/// ship.
...
pub mod descriptor {
    ...
    /// The gRPC transport service definition.
    const TRANSPORT_FILE: &str = "derectransport.proto";

    /// The complete import closure of the DeRec schema: every `.proto` file
    /// this crate was compiled from, plus the well-known types they import.
    ///
    /// Generated by this crate's build script from this crate's sources, so
    /// the schema and the crate version cannot drift apart.
    pub const FILE_DESCRIPTOR_SET: &[u8] =
        include_bytes!(concat!(env!("OUT_DIR"), "/derec_descriptor.bin"));

    /// The closure reduced to the transport service definition alone.
    ///
    /// Pair with `extern_path(".org.derecalliance.derec.protobuf",
    /// "::derec_proto")` so the generated service binds to the message types
    /// this crate already exports. Passing the full closure instead would
    /// emit a second, structurally identical set of message types that does
    /// not unify with the first.
    pub fn transport_descriptor() -> FileDescriptorSet {
        let mut fds = FileDescriptorSet::decode(FILE_DESCRIPTOR_SET)
            .expect("descriptor written by this crate's build script must decode");
        fds.file.retain(|f| f.name() == TRANSPORT_FILE);
        fds
    }
}
