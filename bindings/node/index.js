...
const threshold = 2;
const version = 1;
let shares = protect_secret(secret_id, secret_data, channels, threshold, version);
...
let response = generate_verification_response(secret_id, some_channel, some_share, request);
console.log("generate_verification_response: ", response);
let verification_expected_true = verify_share_response(secret_id, some_channel, some_share, response);
console.log("verify_share_response (expected true): ", verification_expected_true);
let verification = verify_share_response(secret_id, 1n, shares.value.get(2), response);
console.log("verify_share_response (expected false): ", verification);
let share_request = generate_share_request(1n, secret_id, version);
console.log("generate_share_request: ", share_request);
let share_response_1 = generate_share_response(secret_id, 1n, shares.value.get(1), share_request);
console.log("generate_share_response: ", share_response_1);
let share_response_2 = generate_share_response(secret_id, 2n, shares.value.get(2), share_request);
console.log("generate_share_response: ", share_response_2);
let share_response_3 = generate_share_response(secret_id, 3n, shares.value.get(3), share_request);
...
try {
    let recovered = recover_from_share_responses({ "value": responses }, secret_id, version);
...
let create_contact_message_result = create_contact_message(channel_id, "https://example.com/alice");
console.log("create_contact_message: ", create_contact_message_result);
// run by Bob, who scans Alice's QR code
let produce_pairing_request_message_result = produce_pairing_request_message(channel_id, role_helper, create_contact_message_result.contact_message);
...
let produce_pairing_response_message_result = produce_pairing_response_message(role_sharer, produce_pairing_request_message_result.pair_request_message, create_contact_message_result.secret_key_material);
...
let process_pairing_response_message_result = process_pairing_response_message(create_contact_message_result.contact_message, produce_pairing_response_message_result.pair_response_message, produce_pairing_request_message_result.secret_key_material);
