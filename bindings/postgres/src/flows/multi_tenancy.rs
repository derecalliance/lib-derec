...
pub async fn run() {
    println!("=== [Multi-tenancy] vault A and vault B share one Postgres DB ===");
    ...
    assert_eq!(count_channels(&shared_db.client(), VAULT_A).await, 2);
    assert_eq!(count_channels(&shared_db.client(), VAULT_B).await, 2);
    ...
    .await;
    assert_eq!(count_user_secrets(&shared_db.client(), VAULT_A).await, 1);
    assert_eq!(count_user_secrets(&shared_db.client(), VAULT_B).await, 1);
    // Owner-side `shares` rows live in the same shared table, also
    // partitioned by secret_id.
    assert_eq!(count_shares(&shared_db.client(), VAULT_A).await, 2);
    assert_eq!(count_shares(&shared_db.client(), VAULT_B).await, 2);
    println!(
    ...
    assert_eq!(count_channels(&shared_db.client(), VAULT_A).await, 0);
    assert_eq!(count_user_secrets(&shared_db.client(), VAULT_A).await, 0);
    assert_eq!(count_shares(&shared_db.client(), VAULT_A).await, 0);
