...
//! in Postgres, recoverable by a second protocol attached to the same
//! database.
...
pub async fn run() {
    ...
    // their rows are still in Postgres).
    owner
        .protocol
        ...
        .await
        .expect("owner.start(Discovery) must succeed with channels resurrected from Postgres");
