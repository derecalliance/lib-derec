...
async fn every_contact_mode_pairs_and_persists() {
    ...
    .enumerate()
    {
        ...
        let helper_db = Database::open_in_memory();
        let mut owner = Peer::new(owner_db.connection(), "Owner", "https://owner.example.com");
        let mut helper = Peer::new(helper_db.connection(), "Helper", "https://helper.example.com");
        ...
        for (label, db) in [("owner", &owner_db), ("helper", &helper_db)] {
            assert!(
                channel_exists(&db.connection(), DEFAULT_TEST_SECRET_ID, channel.0),
                "{label} must hold the paired channel after a {mode:?} handshake"
            );
            assert_eq!(
                count_secrets(&db.connection(), DEFAULT_TEST_SECRET_ID),
                1,
