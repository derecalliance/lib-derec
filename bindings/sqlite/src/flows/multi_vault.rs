...
pub async fn run() {
    ...
    assert_eq!(count_channels(&user_db.connection(), VAULT_WALLET), 2);
    assert_eq!(count_channels(&user_db.connection(), VAULT_EMAIL), 2);
    ...
    .await;
    assert_eq!(count_user_secrets(&user_db.connection(), VAULT_WALLET), 1);
    assert_eq!(count_user_secrets(&user_db.connection(), VAULT_EMAIL), 1);
    assert_eq!(count_shares(&user_db.connection(), VAULT_WALLET), 2);
    assert_eq!(count_shares(&user_db.connection(), VAULT_EMAIL), 2);
    assert_eq!(count_shares(&alice_db.connection(), VAULT_WALLET), 1);
    assert_eq!(count_shares(&bob_db.connection(), VAULT_WALLET), 1);
    assert_eq!(count_shares(&carol_db.connection(), VAULT_EMAIL), 1);
    assert_eq!(count_shares(&dave_db.connection(), VAULT_EMAIL), 1);
    println!(
    ...
    assert_eq!(
        count_shares(&alice_db.connection(), VAULT_EMAIL),
        0,
        "Alice (wallet helper) must not hold any email-vault shares"
    );
    assert_eq!(
        count_shares(&carol_db.connection(), VAULT_WALLET),
        0,
    ...
    // without a re-pair.
    assert_eq!(
        count_user_secrets(&user_db.connection(), VAULT_EMAIL),
        1,
        "email vault user_secrets row must survive the wallet recovery"
    );
    assert_eq!(
        count_channels(&user_db.connection(), VAULT_EMAIL),
        2,
