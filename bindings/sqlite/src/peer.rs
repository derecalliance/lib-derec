//! Test peer composed of a SQLite-backed protocol + in-process
...
use crate::stores::{
    SqliteChannelStore, SqliteSecretStore, SqliteShareStore, SqliteUserSecretStore,
};
use crate::transport::InProcessTransport;

pub const DEFAULT_TEST_SECRET_ID: u64 = 0xDE_2EC;

pub type SqliteProtocol = DeRecProtocol<
    SqliteChannelStore,
    SqliteShareStore,
    SqliteSecretStore,
...
impl Peer {
    pub fn new(connection: SharedConnection, label: &'static str, uri: &str) -> Self {
        Self::with_options(connection, label, uri, PeerOptions::default())
    }
    ...
    fn with_full_options(
        ...
        let share_store = SqliteShareStore::new(connection.clone());
        let secret_store = SqliteSecretStore::new(connection.clone());
        let user_secret_store = SqliteUserSecretStore::new(connection);
    ...
    pub fn drain(&self) -> Vec<(TransportProtocol, Vec<u8>)> {
        // Returning the tuple shape verbatim keeps the call sites
        // (which destructure inline) readable without leaking the
        // transport-internal envelope alias.
        self.transport.drain()
    }
}

/// Feed `bytes` to `peer.process`, then auto-accept every
/// `ActionRequired` event the same way the other bindings do, so
/// flow-internal action plumbing is invisible to the tests.
...
/// Drain every peer's outbox and route each message to whichever peer
/// owns the destination URI, recursing until the network is
/// quiescent. The `peers` slice owns the routing table; URIs must be
/// unique.
