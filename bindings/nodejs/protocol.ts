...
//
// Each sub-test mirrors the corresponding Rust smoke test in
// bindings/rust/src/protocol.rs and simulates two-party communication by
// having both sides share the same in-process "network" (a RecordingTransport
// whose outbox is drained manually and fed to the other side).

import { DeRecProtocol, SenderKind } from "@derec-alliance/nodejs";
import type { DeRecEvent } from "@derec-alliance/nodejs";
...
class InMemoryShareStore {
  // Nested map: channelId → hexSecretId → version → bytes
  ...
  /**
   * Copy the stored share for (secretId, version) from one channel to another.
   * Simulates the application-layer contact remapping that happens when a
   * recovering Owner re-pairs under a new channel ID.
   */
  copyShare(
...
class RecordingTransport {
  ...
  /** Remove and return all queued outbound messages. */
...
async function runPairingFlow(): Promise<void> {
  ...
  // Normal pairing: Owner creates contact, Helper initiates.
...
async function runSharingFlow(): Promise<void> {
  ...
  // Owner distributes the secret to both helpers (threshold = 2).
...
async function runDiscoveryAndRecoveryFlow(): Promise<void> {
  ...
  // Helper creates a contact for the recovery session.
  // In production, this is delivered to the recovering Owner out-of-band (QR, link…).
  // Application-layer: the Helper also maps its share from the old channel to the
  // recovery channel (equivalent to updating a contact record with a new channel ID).
  ...
  // In production the Owner would authenticate with the Helper out-of-band first.
